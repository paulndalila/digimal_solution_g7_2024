document.addEventListener("DOMContentLoaded", () => {  
  ...
  function processVillage(village_id){
    fetch('https://training.digimal.uonbi.ac.ke/api/get_org_unit_with_children?id='+village_id, {
        ...
        .then(res => {
          //console.log(village_id)
          document.getElementById('supervisor').innerHTML = res.message[0].supervisor;
  ...
  async function locate_village(villageName, org_id, parent_id){
    ...
    const geocodes = await getCoordinatesByOrgId(org_id);

    if(geocodes !=null){
      show_map_and_pin_location('village', villageName, geocodes.lat, geocodes.long, org_id)
    }else{    
      //Supposed to show button for setting different geocodes
      if (isAuthenticated()) {
        document.getElementById('add_village_btn').style.display = 'block';
        add_new_village_fun(villageName,org_id);
      }
      ...
      if (newVillageName) {
        ...
        var input_query1 = villageName.replace(/\bvillage\b/gi, '').trim()+', '+parent_name;
  ...
  async function processUser(level, org_id, parent_id) {
    ...
    if (level === 1) {
      // National level
      fetchDataAndPopulateDropdown("get_sub_counties_list", org_id, "county_select", "County");
    } else if (level === 2) {
      //County level    
